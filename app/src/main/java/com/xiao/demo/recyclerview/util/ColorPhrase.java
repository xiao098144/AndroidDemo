...
/**
 * A fluent API for formatting Strings. Canonical usage: <pre> CharSequence
 * chars = ColorPhrase.from("I'm<Chinese>,I love <China>"). withSeparator("<>").
 * innerColor(0xFFE6454A). outerColor(0xFF666666). format(); </pre> <ul>
 * <li>Surround keys with curly braces; use two {{ to escape.</li> <li>Fails
 * fast on any mismatched keys.</li> </ul> The constructor parses the original
 * pattern into a doubly-linked list of {@link Token}s. These tokens do not
 * modify the original pattern, thus preserving any spans. <p/> The
 * {@link #format()} method iterates over the tokens, replacing and coloring the
 * text as it iterates. The doubly-linked list allows each token to ask its
 * predecessor for the expanded length.
 */
public class ColorPhrase
{
	...
	/**
	 * The constructor parses the original pattern into this doubly-linked list of
	 * tokens.
	 */
	private Token head;
	...
	{
		if (TextUtils.isEmpty(_separator)) { throw new IllegalArgumentException("separator must not be empty!"); }
		if (_separator.length() > 2) { throw new IllegalArgumentException("separator‘s length must not be more than 3 charactors!"); }
	...
	{
		if (separator.length() == 2) { return separator.charAt(1); }
		return separator.charAt(0);
	}
	...
	public CharSequence format()
	{
		if (formatted == null)
		{
			if (!checkPattern()) { throw new IllegalStateException("the separators don't match in the pattern!"); }
	...
	private boolean checkPattern()
	{
		...
		for (int i = 0; i < pattern.length(); i++)
		{
			...
			{
				if (!(!separatorStack.isEmpty() && (separatorStack.pop() == leftSeparator))) { return false; }
			}
		}
	...
	private InnerToken inner(Token prev)
	{
		...
		if (curChar == EOF) { throw new IllegalArgumentException("Missing closing separator"); }
		// consume the right separator.
		consume();

		if (sb.length() == 0) { throw new IllegalStateException("Disallow empty content between separators,for example {}"); }
	 ...
	 * Does not pass through to {@link #format()} because doing so would drop all
	 * spans.
	 */
	@Override
